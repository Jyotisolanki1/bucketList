...
const updateProfile = async (req, res) => {
  try {
    ...
    var updateData = {
      ...
      sales_person: req.body.sales_person ? req.body.sales_person : "",
...
const getService = async (req, res) => {
  try {
    const result = await serviceModel.find({ status: "Active" });
    if (result) {
      return res.json({
        success: true,
...
const updateService = async (req, res) => {
  try {
    ...
    const user = await bucketModel.findOneAndUpdate(
      { _id: new mongoose.Types.ObjectId(req.body.id) },
      { $set: updateData },
      { new: true }
    );
    ...
    if (user) {
      return res.status(200).json({
        responseCode: 200,
        success: true,
        message: "Bucket update successfully.",
        data: user,
      });
    }
  } catch (error) {
...
const deleteSubCategory = async (req, res) => {
  try {
    console.log("testing delete api for sub cateogry delete");
  } catch (error) {
    console.error("Error in getSubCat", error);
    return res.status(500).json({
      responseCode: 500,
