...
import { GetBucketCount } from 'store/slices/service';
...
const EarningCard = () => {
    ...
    const [totalBucket, setTotalBucket] = React.useState();
    ...
    React.useEffect(() => {
        async function fetchData() {
            // You can await here
            setLoading(true);
            const response = await dispatch(GetBucketCount());
            console.log(response);
            if (response?.success === true) {
                setTotalBucket(response.data);
                setLoading(false);
            }
        }
        fetchData();
    }, [totalBucket, dispatch]);
    return (
        <>
            {isLoading ? (
                <SkeletonEarningCard />
