...
import { EditUserAPI } from '../../store/slices/user1';
...
const AddUser = ({ item, open, close }) => {
    const dispatch = useDispatch();
    const PROXY = process.env.REACT_APP_API_URL;
    const Avatar1 = '/assets/images/users/defaultlogo.png';
    const [loading, setLoading] = useState(false);
    const [selectedImage, setSelectedImage] = useState(`${PROXY}/${item?.profile_pic}`);
    ...
    const validationSchema = yup.object({
        profile_pic: yup
            ...
            .test('fileType', 'Invalid file type. Only images are allowed', (value) => {
                if (!value) return true; // Ensures the field is required
    ...
    const onSubmit = async (values, { resetForm }) => {
        if (imageError) {
            formik.setFieldError('profile_pic', 'Please select a valid image file (PNG, JPEG)');
        } else {
            setLoading(true);
            ...
            if (values.profile_pic && values.profile_pic !== null) {
                formData.append('profile_pic', values.profile_pic);
            }
            try {
                const res = await dispatch(EditUserAPI(formData));
                                            ...
                                            <Grid container spacing={1}>
                                                ...
                                                <Grid item xs={12}>
                                                    ...
                                                    <Typography variant="caption" style={{ color: 'red' }}>
                                                        {formik.errors.profile_pic && formik.errors.profile_pic}
                                                    </Typography>
                                                </Grid>
                                            </Grid>
                                        </Grid>
